...
class PaymentMethodController extends Controller
{
    ...
    public function update(Request $request, $name)
    {
        ...
        if ($name == 'digital_payment') {
            ...
            $payment = BusinessSetting::where('type', 'benefit')->first();
                DB::table('business_settings')->where(['type' => 'benefit'])->update([
                    'type' => 'benefit',
                    'value' => json_encode([
                        'status' => $request['status'],
                    ]),
        ...
        } elseif ($name == 'credimax') {
            $payment = BusinessSetting::where('type', 'credimax')->first();
            if (isset($payment) == false) {
                DB::table('business_settings')->insert([
                    'type' => 'credimax',
                    'value' => json_encode([
                ...
                DB::table('business_settings')->where(['type' => 'credimax'])->update([
                    'type' => 'credimax',
                    'value' => json_encode([
                        'status' => $request['status'],
                        'merchant_id' => $request['merchant_id'],
                        'operator_id' => $request['operator_id'],
                        'operator_password' => $request['operator_password']
                    ]),
                    'updated_at' => now()
                ]);
            }
